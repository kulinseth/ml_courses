...
BN_fc2 = True # Set to True to use batch normalization for FC_1 layer
BN_fc3 = True # Set to True to use batch normalization for FC_1 layer
...
class ArtistConvNet:
  ...
  def build_graph(self):
    with self.graph.as_default():
      ...
      x = self.images
      bn1 = tf.layers.batch_normalization(inputs=self.images, axis=3, training=self.training)
      relu1 = tf.nn.relu(bn1)
      conv1 = tf.layers.conv2d(inputs=relu1, filters=3, kernel_size=5, strides=1, padding="same")
      bn2 = tf.layers.batch_normalization(inputs=conv1, axis=3, training=self.training)
      relu2 = tf.nn.relu(bn2)
      conv2 = tf.layers.conv2d(inputs=relu2, filters=3, kernel_size=5, strides=1, padding="same")
      y = conv2 + x
      
      flat = tf.contrib.layers.flatten(inputs=y)      
